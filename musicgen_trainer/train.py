...
def preprocess_audio(audio_path, model: MusicGen, duration: int = 30):
    ...
    # wav = wav.mean(dim=0, keepdim=True) #comment if for stereo
    ...
    # assert wav.shape[0] == 1  #comment if for stereo

    wav = wav.cuda()
    wav = wav.unsqueeze(0) # if for stereo
    # wav = wav.unsqueeze(1) # if for mono
